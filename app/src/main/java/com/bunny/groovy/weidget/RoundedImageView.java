...
public class RoundedImageView extends ImageView {
    ...
    private void updateDrawable(Drawable drawable) {
        ...
        if (drawable instanceof SelectableRoundedCornerDrawable) {
            ((SelectableRoundedCornerDrawable) drawable).setScaleType(mScaleType);
            ((SelectableRoundedCornerDrawable) drawable).setCornerRadii(mRadii);
            ((SelectableRoundedCornerDrawable) drawable).setBorderWidth(mBorderWidth);
            ((SelectableRoundedCornerDrawable) drawable).setBorderColor(mBorderColor);
            ((SelectableRoundedCornerDrawable) drawable).setOval(isOval);
        } else if (drawable instanceof LayerDrawable) {
            LayerDrawable ld = (LayerDrawable) drawable;
            final int num = ld.getNumberOfLayers();
            for (int i = 0; i < num; i++) {
                updateDrawable(ld.getDrawable(i));
            }
        }
    }
            ...
            if (drawable != null) {
                if (drawable instanceof SelectableRoundedCornerDrawable) {
                    ...
                    for (int i = 0; i < num; i++) {
                        ...
                        ld.setId(i, i);
                        ld.setDrawableByLayerId(i, fromDrawable(d, r));
                    }
                    return ld;
                }
