...
public class TimePopupWindow {
    ...
    public void setListener(OnTimeConfirmListener listener) {
     ...
     * 弹出选择时间窗口
     */
    public void showTimeChoosePop(View view) {
        this.mShowView = view;
        if (mTimePop == null)
            initTimePop();
        mTimePop.showAtLocation(view, Gravity.CENTER, 0, 0);
    }
    ...
    private void initTimePop() {
        mTimePop = new PopupWindow(mActivity);
        View timeView = LayoutInflater.from(mActivity).inflate(R.layout.weidget_time_choose_layout, null);
...
//                    etTime.setText(DateUtils.getFormatTime(mSelectDate.getTime(), startTime) + "-" + endTime);
    ...
    private void showDatePop() {
        if (mDatePop == null)
            initDatePop();
        mDatePop.showAtLocation(mShowView, Gravity.CENTER, 0, 0);
    }
    ...
    private void initDatePop() {
        ...
        mDatePop = new PopupWindow(mActivity);
        View dateView = LayoutInflater.from(mActivity).inflate(R.layout.weidget_date_choose_layout, null, false);
